...
- **Anything reacting to the path choice must observe, never drive.** `switchTab()` only persists to `localStorage` when it is invoked from a real click, because it reads the implicit global `event` (`theme.js:120`). So calling `switchTab()` programmatically silently fails to persist, and writing `localStorage` yourself duplicates the theme's contract and will drift from it. The correct hook is a `MutationObserver` on the `.active` class, which sees both user clicks and the theme's own `restoreTabSelections()` on load with no coupling. The locked-path banner in the shared `pathtabs` shortcode is built this way.
