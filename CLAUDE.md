...
- **`scripts/repoConfig.json`'s `deploymentPaths` is the single source of the path vocabulary.** `groupid: deploy-path`; keys `docker` / `k8s`; titles **"Docker Compose"** and **"Kubernetes / Helm"**, in that order. That file is what Hugo reads at build time and therefore what CentralRepo's shortcodes accept (they `errorf` if it is absent). `gen_handouts.py` builds `PATHS` from it at import time — `slug` and `weight` stay generator-local — and `lint_paths.py` imports `PATHS`, the marker regexes and `FENCE_RE` from `gen_handouts.py`. Adding a path is a one-file edit; the two scripts travel as a pair. Both fail fast if the file itself is missing or unparseable — real config errors. A **missing or empty `deploymentPaths` is not an error**: `load_paths()` returns `[]` and every downstream step (generation, `--check`, `lint_paths.py`) no-ops cleanly rather than silently producing zero handouts and letting `--check` pass vacuously. This is what makes it safe for the same four files (both scripts, `path-lint.yml`, `handout-pdf.yml`) to sit inert in a repo that has never opted in — `UserRepo` carries copies for exactly this reason, see `content/02Hugo/7_printable_handouts/index.md` there. `handout-pdf.yml` asks the generator for the slug list (`--list-slugs`) rather than hardcoding it, and gates its expensive steps (image pull, Hugo build, PDF render) behind that list being non-empty.
...
- **A `pathtab`/`pathonly` body being empty is a hard build error (`errorf`), and only Hugo catches it — `lint_paths.py` cannot.** The linter is a line-based markup scanner; an empty tab is syntactically legal to it. Real incident 2026-08-20: PR #27 emptied the k8s `pathtab` on `content/05Security/1_lab/index.md`; `lint` stayed green through #27→#28→#29 and the Pages deploy broke only after merge, on the push-triggered Hugo build. This is why `handout-pdf.yml` (which also runs a full Hugo build) was made `pull_request`-triggered — see below — rather than trying to teach the linter to validate emptiness.
- **`path-lint.yml` and `handout-pdf.yml` both run on `pull_request` now, and `main` has `enforce_admins: true`.** Neither was true before 2026-08-20: `path-lint.yml` ran on PRs but only checks markup and handout freshness, not whether the site actually builds; `handout-pdf.yml` (the one that runs a real Hugo build) was `push: branches: [main]`-only, so a build-breaking change surfaced only after merge; and admins could push straight to `main`, bypassing PR review entirely, with GitHub only warning ("Bypassed rule violations..."), not blocking. All three closed together, deliberately — closing only the bypass without the pre-merge build check would just move the same failure from "unreviewed direct push" to "reviewed PR that still breaks the site." A direct push to `main` — admin or not — now gets a hard `[remote rejected]`; the only way in is a PR with `lint` and `handouts` both green.
- **`path-lint.yml`'s `lint` job auto-fixes stale handouts on PRs, pushed via `HANDOUT_AUTOFIX_PAT`, not `GITHUB_TOKEN`.** A `GITHUB_TOKEN` push deliberately does not retrigger workflow runs (GitHub's own recursion guard), so an auto-fix commit made with it would never get its own green check — the PR would stay pinned to the pre-fix, stale-handout status. `HANDOUT_AUTOFIX_PAT` is a fine-grained PAT scoped to `contents: write` on this repo only, stored as a repo secret, and its push does retrigger checks. If the secret is ever empty (e.g. this workflow file copied into a repo — `UserRepo` — that hasn't provisioned its own PAT), the step reverts the working tree and no-ops instead of erroring, so `lint_paths.py`/`--check` still report the real "stale, regenerate locally" failure rather than an opaque git-auth error.
