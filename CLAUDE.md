...
- **Never put `[skip ci]` in a commit message on a branch destined for a squash merge — it propagates into the merge commit and silently suppresses the Pages deploy.** GitHub concatenates every squashed commit's message into the merge commit body and honours `[skip ci]` found anywhere in it. Plan/log commits use `[skip ci]` by convention, so PR #19 merged to `main` and **no workflow ran at all** — no failed run, no annotation, nothing to notice; `main` simply carried code the published site did not. Recovery is `gh workflow run static.yml --repo FortinetCloudCSE/ai-101 --ref main`, but the habit is what matters: after merging any PR here, confirm a `Deploy static content to Pages` run exists for the merge SHA. Absence of a run is the failure mode, and it looks exactly like success.
